...
def post_process_clir_queries(queries, source):
    """
    In CLIR queires, query index is formulated with <qid#qlang#qsource>
    """
    queries_processed = {}
    
    for query_index_info, query_text in queries.items():
        qid, qlang, qsource = query_index_info.split('#')
        if qsource == source:
            queries_processed[qid] = query_text
